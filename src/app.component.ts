    ...
    if (this.dataService.isAdmin()) {
      ...
      if (user && user.role === 'SuperAdmin') {
        ...
        return this.platformNavItems.filter(nav => {
          ...
          if (nav.path.includes('callcenter')) return perms.includes('MANAGE_MODERATION') || perms.includes('VIEW_DASHBOARD'); // Needs dedicated perm ideally, routing broadly for now
