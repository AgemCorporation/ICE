                ...
                <div class="p-6 pb-[calc(6rem+env(safe-area-inset-bottom))]">
                   ...
                   <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 flex flex-col items-center mb-6 relative overflow-hidden text-center">
                      ...
                      <div class="relative mb-4 group">
                         ...
                         @if (currentClientData()?.avatarUrl) {
                            <button (click)="deleteAvatar()" title="Supprimer la photo" class="absolute bottom-0 left-0 w-8 h-8 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center shadow-lg border-2 border-white dark:border-slate-900 transition-transform active:scale-90 z-10">
                               <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export class MobileAppComponent {
   ...
   async deleteAvatar() {
      ...
      const confirmDelete = window.confirm('Voulez-vous vraiment supprimer votre photo de profil ?');
      if (!confirmDelete) return;

      this.toastService.show('Suppression en cours...', 'info');
      try {
         const updatedUser = await this.dataService.updateAvatar(clientId, '');
         if (updatedUser) {
            this.currentClientData.set(updatedUser);
         }
         this.toastService.show('Photo supprimée', 'success');
      } catch (err) {
         console.error('Delete avatar API failed:', err);
         this.toastService.show('Erreur de réseau ou serveur', 'error');
      }
   }
